  ...
  return (
    <section className="space-y-4">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="font-display text-3xl font-bold text-steel-50">Match Verification</h1>
          <p className="text-sm text-steel-300">Review AI comparisons and verify possible matches.</p>
        </div>
        <button
          onClick={() => fetchMatches(true)}
          disabled={loading}
          className="flex items-center gap-2 rounded-xl border border-steel-600/85 bg-steel-900/80 px-4 py-2 text-sm font-medium text-steel-200 hover:bg-steel-800/45"
        >
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`rounded-xl px-4 py-2 text-sm font-semibold transition-colors ${filter === 'pending' ? 'bg-navy-900 text-white' : 'bg-steel-800/75 text-steel-200 hover:bg-steel-700/80'}`}
        >
          Pending {pendingCount > 0 && <span className="ml-1 rounded-full bg-red-500/100 px-1.5 py-0.5 text-xs text-white">{pendingCount}</span>}
        </button>
        <button
          onClick={() => setFilter('all')}
          className={`rounded-xl px-4 py-2 text-sm font-semibold transition-colors ${filter === 'all' ? 'bg-navy-900 text-white' : 'bg-steel-800/75 text-steel-200 hover:bg-steel-700/80'}`}
        >
          All ({matches.length})
        </button>
      </div>
      ...
      {loading ? (
        ...
        <p className="rounded-xl bg-steel-800/75 p-4 text-sm text-steel-200">
        ...
        <div className="space-y-4">
          {displayed.map((match) => {
            ...
            return (
              <article key={match.id} className="rounded-2xl border border-steel-700/80 bg-steel-900/80 p-5 shadow-card">
                <div className="grid gap-4 md:grid-cols-2">
                  <div>
                    <p className="mb-2 text-sm font-semibold text-steel-300">Missing Person Photo</p>
                    {match.missingPhoto ? (
                      <img src={match.missingPhoto} alt={match.personName} className="h-56 w-full rounded-xl object-cover" />
                    ) : (
                      <div className="flex h-56 items-center justify-center rounded-xl bg-steel-800/75 text-sm text-steel-400">Photo unavailable</div>
                    )}
                  </div>
                  <div>
                    <p className="mb-2 text-sm font-semibold text-steel-300">Sighting / CCTV Frame</p>
                    {match.cctvPhoto ? (
                      <img src={match.cctvPhoto} alt={`${match.personName} sighting`} className="h-56 w-full rounded-xl object-cover" />
                    ) : (
                      <div className="flex h-56 items-center justify-center rounded-xl bg-steel-800/75 text-sm text-steel-400">Frame unavailable</div>
                    )}
                  </div>
                </div>

                <div className="mt-4 grid gap-2 rounded-xl bg-steel-800/45 p-3 text-sm text-steel-200 sm:grid-cols-3">
                  ...
                  <p className="sm:col-span-2">
                    ...
                    <span className={`font-semibold ${match.status === 'Approved' ? 'text-emerald-600' : match.status === 'Rejected' ? 'text-red-300' : 'text-amber-600'}`}>
                      {match.status}
                    </span>
                  </p>
                </div>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  ...
                  <button
                    ...
                    disabled={isDeleting}
                    className="ml-auto flex items-center gap-1.5 rounded-xl bg-red-500/18 px-4 py-2 text-sm font-semibold text-red-300 hover:bg-red-200 disabled:opacity-60"
                  >
